    ...
    : QDialog(parent), m_manager(manager)
{
    ...
    m_grandTotalLabel->setStyleSheet(
        "font-size: 14px;"
        "font-weight: bold;"
        "color: white;"
        );
...
void PurchaseWindow::refreshScreen()
{
    ...
    for (const QString& stadium : stadiumNames)
    {
        ...
        m_purchaseList->addItem(
            QString("Stadium Total: $%1")
                .arg(m_manager->getStadiumTotal(stadium), 0, 'f', 2)
            );
    }

    m_grandTotalLabel->setText(
        QString("Grand Total: $%1")
            .arg(m_manager->getGrandTotal(), 0, 'f', 2)
        );
}
